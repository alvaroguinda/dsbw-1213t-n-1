...
class EnquestesApi(enquestesService:EnquestesService) extends Api {
  ...
  val getPreguntaAdmin = "GET /api/enquestes/admin([0-9]+)/enq([a-zA-z0-9]+)/preg([a-zA-z0-9]+)".r
  ...
  val respondreEnquestaUserFinal = "PUT /api/enquestes/user([a-zA-z0-9]+)/enq([a-zA-z0-9]+)/finish".r
  val respondreEnquestaUserFinalPost = "POST /api/enquestes/user([a-zA-z0-9]+)/enq([a-zA-z0-9]+)/finish".r
  ...
  def service(method: String, uri: String, parameters: Map[String, List[String]] = Map(), headers: Map[String, String] = Map(), body: Option[JSON] = None, session: HttpSession): Response = {
    try {
      (method + " " + uri) match {
        ...
        case getPreguntaAdmin(idUser,idEnquesta, idPregunta) => Response(HttpStatusCode.Ok, enquestesService.getPregunta(idUser,idEnquesta, idPregunta))
        ...
        case respondreEnquestaUserFinal(idUser,idEnquesta) => Response(HttpStatusCode.Ok, enquestesService.putRespondreEnquestaFinalPut(idUser,idEnquesta,JSON.fromJSON[Respostes](body.getOrElse(throw new Exception("Bad Request")))))
        case respondreEnquestaUserFinalPost(idUser,idEnquesta) => Response(HttpStatusCode.Ok, enquestesService.putRespondreEnquestaFinalPost(idUser,idEnquesta,JSON.fromJSON[Respostes](body.getOrElse(throw new Exception("Bad Request")))))
